...
sealed interface ViewSubjectEvents {
    ...
    data class OnCreateModule(val module : Modules,val result: (UiState<String>) -> Unit): ViewSubjectEvents
    ...
    data class UpdateLock(val moduleID : String,val lock : Boolean,val context: Context) :ViewSubjectEvents

    data class OnCreateActivity(val activity: Activity, val result  : (UiState<String>) -> Unit) : ViewSubjectEvents
